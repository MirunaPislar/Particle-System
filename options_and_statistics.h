...
void printDataOnScreen()
{
  ...
  // FPS is not always the best way to measure performance because it is not linear in the time domain
  // https://www.khronos.org/opengl/wiki/Performance#Measuring_Performance
  // The Frame Time is not proportional to the amount of things that you render.
  frameTime = 1.0 / framesPerSecond;
  sprintf(string, "Frame Time: %.2f seconds", frameTime);
  drawString(GLUT_BITMAP_9_BY_15, -350, 500 - 7 * 14, string);
